      ...
      {records.map((r) => {
        ...
        return (
          <a
            ...
            className={`flex gap-3 items-center px-3 py-1.5 rounded no-underline text-gray-800 my-0.5 hover:bg-gray-100 ${
              r.won ? "border-l-[3px] border-l-green-600" : "border-l-[3px] border-l-red-600"
            }`}
          >
            <span className="min-w-[80px] text-gray-500 text-[13px]">{formatMatchDate(r.date)}</span>
            <span className="font-semibold min-w-[50px] text-center">{r.mapScore}</span>
            <span className="flex-1">vs {r.opponentName}{opponentElo}</span>
            {meta.length > 0 && <span className="text-xs text-gray-400">{meta.join(" \u00B7 ")}</span>}
          </a>
        )
      })}
    </div>
