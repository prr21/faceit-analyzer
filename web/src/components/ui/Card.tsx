...
interface CardProps {
  title: string
  value: string | number
  valueColor?: "green" | "red" | string
  subtitle?: string
  children?: ReactNode
}

export function Card({ title, value, valueColor, subtitle, children }: CardProps) {
  const colorClass = valueColor === "green" ? "text-green-600 dark:text-green-400" : valueColor === "red" ? "text-red-600 dark:text-red-400" : valueColor ?? ""
  ...
  return (
    <div className="flex-1 min-w-[100px] sm:min-w-[140px] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 sm:p-4 text-center">
      ...
      {subtitle && <div className="text-xs text-gray-400 dark:text-gray-400 mt-1">{subtitle}</div>}
      {children}
    </div>
  )
}
